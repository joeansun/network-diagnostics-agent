...
_TIME_RE_UNIX = re.compile(
    r"\btime[=<]\s*(?P<ms>[\d().]+)\s*ms\b", 
    re.IGNORECASE
)

_ADDR_RE_UNIX = re.compile(
    r"^---\s+(?P<addr>.+?)\s+ping statistics\s+---$", 
    re.IGNORECASE
)
...
_RTT_RE_UNIX = re.compile(
    ...
    r"(?P<min>\d+(?:\.\d+)?)/(?P<avg>\d+(?:\.\d+)?)/(?P<max>\d+(?:\.\d+)?)/(?P<std>\d+(?:\.\d+)?)",  # ← Fixed: \. not .
    re.IGNORECASE
)
